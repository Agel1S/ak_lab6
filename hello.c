...
MODULE_DESCRIPTION("Computer architecture Lab 6");
...
MODULE_PARM_DESC(num_of_greet, "'Hello, World!' message will be printed mentioned times");
...
static int __init hello_init(void)
{
    if (num_of_greet == 0 || (num_of_greet >= 5 && num_of_greet <= 10)) {
        printk(KERN_WARNING "Taken parameter is 0 or between 5 and 10");  
    }

    if (num_of_greet > 10) {
        printk(KERN_ERR "Taken parameter is too large");
        return -EINVAL;
    }
